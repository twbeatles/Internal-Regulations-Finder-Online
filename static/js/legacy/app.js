...
export const ReaderMode = {
    ...
    openCitation(citation) {
        if (!citation) return;
        const title = [citation.source, citation.article_no, citation.article_title]
            .filter(Boolean)
            .join(' ');
        const content = citation.excerpt || '';
        this.openItem({
            source: title || citation.source || '규정',
            content,
...
// 검색 결과 키보드 탐색
export const SearchResultNavigator = {
    cards: [],
...
// 파일 목록 로드 (검색 필터용)
// ============================================================================
export async function loadFileListForFilter(selectedValue = null) {
...
export function toggleBookmarkByIndex(index, buttonElement) {
...
// 이벤트 위임으로 북마크 버튼 처리
export function setupBookmarkEventDelegation() {
 ...
 */
export function createResultCard(item, index) {
    const score = Math.round((item.score || 0) * 100);
...
export async function initAdmin(options = {}) {
...
export async function uploadFolderZip(file) {
...
export function handlePreviewEsc(e) {
...
export async function loadRagSettings() {
    const providerEl = document.getElementById('rag-provider');
    if (!providerEl) return;

    try {
        const [settingsRes, statusRes, modeRes] = await Promise.all([
            fetch('/api/settings/rag'),
            fetch('/api/rag/status'),
            fetch('/api/settings/search-mode')
        ]);
        const settingsJson = await settingsRes.json();
        const statusJson = await statusRes.json();
        const modeJson = await modeRes.json();

        const rag = settingsJson?.data?.rag || settingsJson?.rag || {};
        const llm = rag.llm || {};
        providerEl.value = llm.provider || 'ollama';
        const modelEl = document.getElementById('rag-model');
        const baseUrlEl = document.getElementById('rag-base-url');
        const modeEl = document.getElementById('search-mode-select');
        if (modelEl) modelEl.value = llm.model || '';
        if (baseUrlEl) baseUrlEl.value = llm.base_url || 'http://127.0.0.1:11434';
        if (modeEl) {
            modeEl.value = modeJson?.data?.search_mode || modeJson?.search_mode || 'rag';
        }

        const statusEl = document.getElementById('rag-llm-status');
        if (statusEl) {
            const llmStatus = statusJson?.data?.llm || statusJson?.llm || {};
            const primary = llmStatus.primary || {};
            const healthy = primary.healthy ? '연결됨' : '미연결';
            statusEl.textContent = `LLM(${primary.provider || '-'}/${primary.model || '-'}): ${healthy}`;
        }
    } catch (e) {
        const statusEl = document.getElementById('rag-llm-status');
        if (statusEl) statusEl.textContent = 'RAG 설정 로드 실패';
    }

    const saveBtn = document.getElementById('save-rag-settings-btn');
    if (saveBtn && !saveBtn.dataset.bound) {
        saveBtn.addEventListener('click', async () => {
            const payload = {
                rag: {
                    llm: {
                        provider: document.getElementById('rag-provider')?.value || 'ollama',
                        model: document.getElementById('rag-model')?.value || '',
                        base_url: document.getElementById('rag-base-url')?.value || 'http://127.0.0.1:11434'
                    }
                }
            };
            const mode = document.getElementById('search-mode-select')?.value || 'rag';
            saveBtn.disabled = true;
            try {
                const ragRes = await fetch('/api/settings/rag', {
                    method: 'POST',
                ...
                const modeRes = await fetch('/api/settings/search-mode', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'same-origin',
                ...
                const modeJson = await modeRes.json();
                if (ragJson.success && modeJson.success) {
                    Toast.success('RAG 설정 저장', '설정이 적용되었습니다.');
                    await loadRagSettings();
                } else {
                    Toast.error('저장 실패', ragJson.message || modeJson.message || '오류');
                }
            } catch (err) {
...
export async function manageTags(fileId, filename = '') {
...
export async function showRevisions(fileId, filename = '') {
