...
- 문서/코드 기준 고정:
  - `static/app.js`는 점검 시점 기준 3320 lines.
  - `BUILD.md` Lite 예상 크기 표기는 `60-100MB`로 정합화.
- 공개 API/런타임 동작 변경 없음(빌드 패키징 정책만 조정).

## ✅ 정합성 점검 메모 (2026-03-09)

- `server.spec`, `internal_regulations.spec`, `regulation_search.spec`, `regulation_search_gui.spec`의 hiddenimports를 코드의 optional import 경로와 동기화.
  ...
  - run.py 기반 spec에는 `onnxruntime` 및 `app.services.embeddings_backends` 수집 경로를 보강.
- 문서 경로 표기를 현재 저장소 명(`Internal-Regulations-Finder-Online`) 기준으로 정리.
