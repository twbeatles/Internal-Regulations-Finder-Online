...
| 🔌 **오프라인 모드** | 폐쇄망 환경에서 로컬 모델로 작동 |
| 📂 **콘텐츠 접기** | 긴 검색 결과 접기/펼치기 |

---

## 🚀 v1.7 주요 업데이트 (2025-12-25)

### 🔧 코드 품질 및 디버깅
- **버전 일관성 수정**: 모든 파일의 버전 표기 통일
- **코드 리뷰 완료**: 전반적인 코드 품질 검증
- **문서 업데이트**: README.md 및 .spec 파일 최신화

### ✅ 검증된 기능
- XSS 방지 (이벤트 위임 패턴)
- 오프라인 모드 (HuggingFace 환경변수)
- 동적 연도 표시 (푸터)
- 스레드 안전성 (RLock, Semaphore)

---

## 🚀 v1.6 주요 업데이트 (2025-12-25)

### 🛡️ 보안 강화
- **XSS 취약점 수정**: onclick 인라인 핸들러를 이벤트 위임 패턴으로 교체
- **안전한 데이터 전달**: data 속성 기반 인덱스 참조 방식 도입

### 🎨 UI/UX 개선
- **콘텐츠 접기/펼치기**: 300자 이상 결과에 접기 버튼 표시
- **북마크 버튼 개선**: 호버 효과 및 팝 애니메이션 추가
- **접근성 강화**: ARIA 속성 보완, 포커스 표시 개선
- **GUI 창 크기 조정**: 최소 크기 600x700으로 증가

### ⚙️ 코드 품질 개선
- **이벤트 위임 패턴**: 북마크, 복사, 접기 버튼에 적용
- **HuggingFace 환경변수**: 오프라인 모드에서 네트워크 요청 완전 차단
- **스켈레톤 애니메이션**: 로딩 상태 시각적 개선

---

## 📝 v1.5 업데이트 (2025-12-25)

### 🔌 오프라인 모드 (폐쇄망 지원)
- **로컬 모델 사용**: 인터넷 없이 사전 다운로드된 모델로 작동
- **모델 다운로드 스크립트**: `download_models.py`로 모델 사전 다운로드
- **GUI 설정 통합**: 오프라인 모드 체크박스 및 모델 경로 설정
- **친절한 오류 메시지**: 오프라인 모드에서 모델 없을 때 해결 방법 안내

### 설정 파일 확장
- `offline_mode`: 오프라인 모드 활성화 여부
- `local_model_path`: 사전 다운로드된 모델 폴더 경로

---

## 📝 v1.4 업데이트 (2025-12-23)

### 웹 UI/UX 개선
- **북마크 애니메이션**: 향상된 호버 효과 및 팝 애니메이션
- **시스템 메트릭 표시**: 관리자 페이지에서 CPU, 메모리, 활성 검색 수 실시간 조회
- **접근성 개선**: 감소된 모션 미디어 쿼리 지원, 프린트 스타일 추가
- **반응형 레이아웃**: 태블릿 뷰포트 최적화
- **동적 연도**: 푸터 저작권 연도 자동 업데이트

### GUI 앱 개선
- **시스템 메트릭 패널**: CPU, 메모리, 활성 검색 실시간 표시
- **로그 필터링**: INFO/WARNING/ERROR 레벨별 필터 체크박스
- **색상 인디케이터**: 시스템 부하에 따른 색상 변경 (정상: 녹색, 경고: 노랑, 위험: 빨강)
- **버전 업데이트**: GUI 앱 v1.2

### API 개선
- **상태 API 강화**: `/api/status`에 CPU/메모리 사용량, 검색 큐 상태 포함
- **psutil 통합**: 시스템 리소스 모니터링

---

## 📝 v1.3 업데이트 (2025-12-19)
...
├── download_models.py # 모델 사전 다운로드 도구 (오프라인용)
├── requirements.txt   # 패키지 의존성
├── templates/         # HTML 템플릿
├── static/            # CSS, JavaScript
├── uploads/           # 업로드된 규정 파일
├── config/            # 설정 파일 (오프라인 설정 포함)
└── models/            # AI 모델 캐시
```
...
인터넷이 연결되지 않은 사내망에서도 검색 시스템을 운영할 수 있습니다.

### 1. 모델 사전 다운로드 (인터넷 연결 환경)
...
`models` 폴더를 폐쇄망 서버로 복사합니다.

### 3. 설정 변경

**방법 A: GUI에서 설정**
1. 서버 GUI 실행
2. "오프라인 모드 활성화" 체크박스 선택
3. 모델 경로 지정 (선택사항)
4. 서버 재시작
...
{
  "offline_mode": true,
  "local_model_path": "D:/path/to/models"
}
```

### 4. 서버 실행

오프라인 모드가 활성화되면 서버 로그에 다음이 표시됩니다:
```
🔒 오프라인 모드 활성화 - 로컬 모델만 사용
📂 모델 경로: D:/path/to/models
```
...
- 🔌 **오프라인 모드 지원**
  - 폐쇄망 환경에서 로컬 모델 사용
  - `download_models.py` 스크립트 추가
  - GUI에 오프라인 모드 설정 UI 추가
  - `settings.json`에 `offline_mode`, `local_model_path` 옵션 추가

### v1.4 (2025-12-23)
- 🎨 **웹 UI/UX 개선**
  - 북마크 버튼 애니메이션 강화
  - 관리자 페이지 시스템 메트릭 표시 (CPU, 메모리, 활성 검색)
  - 접근성 개선 (prefers-reduced-motion, 프린트 스타일)
  - 반응형 태블릿 레이아웃 최적화
  - 동적 저작권 연도 표시
- 🖥️ **GUI 앱 개선**
  - 시스템 메트릭 패널 (CPU/메모리/활성 검색 실시간 표시)
  - 로그 필터링 (INFO/WARNING/ERROR)
  - 시스템 부하에 따른 색상 인디케이터
- 📡 **API 개선**
  - `/api/status`에 CPU/메모리 사용량, 검색 큐 상태 포함
  - psutil 통합 (시스템 리소스 모니터링)
