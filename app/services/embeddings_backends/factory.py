...
def create_embeddings(
    ...
    """
    # 설정값 결정
    backend = backend if backend is not None else getattr(AppConfig, 'EMBED_BACKEND', 'torch')
    normalize = normalize if normalize is not None else getattr(AppConfig, 'EMBED_NORMALIZE', True)
    ...
    logger.info(f"임베딩 생성 시작: backend={backend}, model={model_name}, normalize={normalize}")
