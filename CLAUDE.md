...
- Optional dependency behavior is now part of the expected context:
  - `flask-cors` may be absent in some environments; the app can fall back to an allowlist-based CORS response path.
  - `waitress` is still required for production, but development can fall back to Flask when missing.
  - OCR dependencies remain optional and should not be assumed in lite/offline builds.
- New repo-level quality guards:
  - `scripts/check_utf8.py`
  - `pyrightconfig.json`
  - `tests/test_optional_dependencies.py`
  - `tests/test_file_routes_by_id.py`
- Packaging consistency expanded beyond the five main GUI/Lite specs:
...
- **프로젝트**: `Internal-Regulations-Finder-Online`
...
>>>>>>> 4186410 (docs: 사용자 중심 사용 가이드 위주로 README.md 전면 개편 (v2.8.3))
